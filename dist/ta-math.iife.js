var TA = (function () {
  ...
  function pointwise(operation) {
    for (var _len = arguments.length, serieses = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    ...
    for (var i = 0, len = serieses[0].length; i < len; i++) {
      ...
      result[i] = operation.apply(void 0, _toConsumableArray(iseries(i)));
    }
  ...
  function psar($high, $low, stepfactor, maxfactor) {
    ...
    for (var i = 2, len = $high.length; i < len; i++) {
      ...
      if (isUp && $low[i] < cursar || !isUp && cursar > $high[i]) {
        isUp = !isUp;
        factor = stepfactor;
        cursar = isUp ? Math.min.apply(Math, _toConsumableArray($low.slice(i - 2, i + 1))) : Math.max.apply(Math, _toConsumableArray($high.slice(i - 2, i + 1)));
  ...
  function obv($close, $volume, signal) {
    ...
    return {
      line: obv,
      signal: sma(obv, signal)
    };
  }
  function roc($close, window) {
    var result = new Array(window).fill(NaN);

    for (var i = window, len = $close.length; i < len; i++) {
  ...
  function stoch($high, $low, $close, window, signal, smooth) {
    var lowest = rolling(function (x) {
      return Math.min.apply(Math, _toConsumableArray(x));
    }, window, $low);
  ...
  function stochRsi($close, window, signal, smooth) {
    ...
    var extreme = rolling(function (x) {
      return {
        low: Math.min.apply(Math, _toConsumableArray(x)),
        high: Math.max.apply(Math, _toConsumableArray(x))
      };
    }, window, _rsi);
