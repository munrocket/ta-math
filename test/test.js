...
describe('EBB', () => {
  let ebb = noize.ebb();  it('Finite test', () => assert.ok((ebb.lower.every(isFinite) && ebb.middle.every(isFinite) && ebb.upper.every(isFinite))));
});
...
describe('EXPDEV', () => {
  let ed = noize.expdev();  it('Finite test', () => assert.ok(ed.every(isFinite)));
});
