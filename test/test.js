...
tape('SMA', (t) => {
  ...
  let actual = new TA([c,c,c,c,c,c], simpleFormat).sma(10).all();
...
tape('EMA', (t) => {
  ...
  let actual = new TA([c,c,c,c,c,c], simpleFormat).ema(10).all();
  t.ok(actual.every(isFinite), 'Finite test');
  let delta = nrmsd(expected.slice(9), actual.slice(9));
  t.ok(delta < 1e-2, `NRMSD test (${delta.toFixed(5)})`);
  t.end();
...
tape('STDDEV', (t) => {
  ...
  let actual = new TA([c,c,c,c,c,c], simpleFormat).stddev(10).all();
  t.ok(actual.every(isFinite), 'Finite test');
...
tape('BB', (t) => {
  ...
  let bb = new TA([c,c,c,c,c,c], simpleFormat).bb(20,2).all();
  t.ok((bb.every(x => isFinite(x.lower)) && bb.every(x => isFinite(x.middle)) && bb.every(x => isFinite(x.upper))), 'Finite test');
  let delta = nrmsd(expected.slice(19), bb.map(x => x.lower).slice(19));
...
// tape('EXPDEV', (t) => {
//   let ed = noize.expdev();;
//   t.ok(ed.every(isFinite), 'Finite test');
//   t.end();
// })
...
tape('PSAR', (t) => {
  ...
  let actual = new TA([h,h,h,l,l,l], simpleFormat).psar().all();
  t.ok(actual.every(isFinite), 'Finite test');
